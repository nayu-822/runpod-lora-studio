...
- manifest warningを記録する最初のDB transactionで、修復後のterminal statusと固定error codeを`manifest_target_status`／`manifest_target_error_code`へ保存します。0036 migrationは既存の0035 repair recordからこのintentを復元し、0037 migrationはitem状態から曖昧なlegacy targetを再分類します。0038 migrationは、旧アプリがFAILED manifestをDB確定済みのまま残した安全なケースだけを`STALE`／`PENDING` repairへ戻し、filesystemには触れません。
...
- terminal jobのmanifest orphanは、DBの`manifest_relative_path`を唯一の保持対象として起動時とschedulerでboundedに再確認します。実行中・repair中・worker/token残存中は対象外とし、generation basename、regular file、project root、symlink、Linux fd identityを検証してからtemp/finalを削除します。DB更新transactionをfilesystem操作中に保持せず、失敗時は固定warningを保存して次回再試行します。
...
manifestの参照commit後も`PENDING`／`REPAIRING` intentをterminal commitまで保持し、旧workerの遅延renameは次回のterminal orphan reconciliationで回収します。0037／0038のmigrationと、起動時・schedulerの最大32件／設定時間内のreconciliationで、DB参照中のmanifestを削除せず、旧generationのfinal／tempだけを安全に整理します。

最新HEADでは`ruff format --check .`、`ruff check .`、`mypy src`、`pytest`が成功し、`pytest`は386 passed、44 skipped、92 warningsでした。skipはWindows環境で利用できないLinux固有のfd／symlink raceテストです。
