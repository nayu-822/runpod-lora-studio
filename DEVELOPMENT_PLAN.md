...
- manifest保存はRunPod Linuxのプロジェクトルートからfdで一段ずつ`O_DIRECTORY|O_NOFOLLOW`付きでトラバースし、ディレクトリ作成ごとに子ディレクトリのfstatと親ディレクトリのfsyncを行う。保持したmanifestディレクトリfd内で一時ファイルを作成・fsyncし、claim再確認、basenameのみのatomic rename、ディレクトリfsync、DB更新、保存後検証の順序を守る。post-commitの参照解除はrowcountと別sessionの参照再確認で状態を分類し、DB参照が残る・commit結果が不明・DBエラーのときはfileを保持し、解除確認後だけunlinkとmanifest directory fsyncを行う。非対応OSは本番経路として扱わず、Linux以外のローカル開発では既存の検証済みフォールバックを使う
- 終端化時の`.part`削除はitem／attemptの終端化と同じトランザクションで`PART_CLEANUP_PENDING`を記録し、commit後に専用claim token／時刻を取得する。Phase Aでclaimをcommitしてsessionを閉じ、Phase Bで別sessionの所有権再確認後にファイル操作を行い、Phase Cで固定warningまたは成功を短いtransactionへ保存する。ファイル操作中にSQLite write transactionを保持しない。cleanup失敗、worker停止、claim喪失で保留状態を失わず、起動時は最大4 batch・最大5秒、retry backoff付きのbounded drainを実行し、stale recoveryもDB commit後に対象item IDを渡してcleanupする。`FAILED retryable=true`と`CANCELED`の`.part`は`get_job()`や再開準備で削除せず、resume時はcleanup claimを原子的に解除または再開を拒否する

Phase 8Bの実装とレビュー指摘対応は、追加テストと品質チェックの成功を確認済み。manifestのfdトラバーサルではproject、acquisition、jobs、job、manifests各ディレクトリのdevice／inodeを保持し、DB更新前後に再トラバースして通常ディレクトリ置換とsymlink置換を検知する。`.part` cleanupはcommit済みclaim、別session再確認、ファイル操作、結果commitの3 phaseと、再開可能な保留状態、retry backoff付きbounded drainを持つ。stale recoveryはjob／item更新をcommitしてからcleanupを実行する。既存の`0032_phase8b_part_cleanup_warnings`と`0033_phase8b_part_cleanup_claims`を変更せず、子migration `0034_phase8b_cleanup_retry_schedule`でcleanup retry時刻とschedule indexを追加した。Google Driveへの成果物同期、完了manifestのDrive保存、PodのStop／TerminateはPhase 9で実装する。
...
Phase 9Aでは、exit code 0で終了した学習jobについて、入力・最終LoRA・dataset snapshot・base modelを再検証し、ローカルexportを作成してからGoogle Driveへ確定同期する。学習時に`TrainingJobRecord.config_snapshot`へ保存したimmutable execution configを実行provenanceの正本とし、completion時の可変`TrainingConfigRecord`は関連IDの検証だけに使用する。同期は`rclone copy`を使用し、artifact本体、remote artifact検証、transfer manifestのupload・再読・完全検証、completion manifestの順序を固定する。completion manifestはremote artifact検証後に最後にコピーし、remote側の再検証とDBのcompleted確定が完了条件である。

- 最終LoRAはimmutable execution snapshotの`output_name`から解決した`output/{output_name}.safetensors`だけを採用し、checkpoint代用、symlink、変化中ファイル、safetensors検証失敗を拒否する。runtime configはjob snapshotとの一致を検証し、remote export configは絶対local path、Python実行ファイル、model／dataset path、秘密情報を含まないsafe configへ変換する。
...
- 起動時のrecoveryはDBのbounded列挙とworker enqueueに限定し、`create_app`の同期経路ではhash、ローカルコピー、rclone、remote marker読み取りを行わない。`reconcile_remote(time_budget_seconds=...)`は最大256件を時間予算内でexecutorへ渡し、blockingなremote処理はworkerで実行する。
- stale遷移は対象status、worker ID、claim token、generation、heartbeat cutoffを含むCASで確定し、heartbeatが有効なworkerをstale化しない。claim喪失後のheartbeat・remote verify・marker upload・completed更新にはgeneration fencingを適用する。
- POSIXのlocal exportはheld directory fd、`O_DIRECTORY|O_NOFOLLOW`、device／inode identity、`O_EXCL`、source安定性、親identity、fsync、atomic rename、final再トラバースを使用する。Windowsでは検証付きfallbackを使用し、cleanupはowned/staleな`.creating-*`だけを対象とする。
- completion markerはサイズ上限、同一remote bytesの再読SHA-256、DB記録hash、全provenance、完全な`export_files`集合、artifact transfer job、remote artifact実体を検証してからDBをcompletedへ更新する。transfer manifestはverification policyに関係なくschema、job／project／run、destination、status、完全なitem path集合、size、local SHA-256、transfer／verification statusを検証し、rcloneの転送時サイズ上限とlisting・download後のサイズ確認を行う。既存markerのrestart/recoveryでもtransfer manifestを再検証し、marker記載の完全一致transfer jobだけを受け入れる。
- Fake storageのoverwrite／cancel／block／failure／exception／remote hash改変注入と、起動enqueue、CAS、marker改変、transfer job不一致、filesystem swapの回帰テストを追加した。Phase 9Aレビュー対応はこの境界を含めて完了し、Phase 9B（RunPod Stop／Terminate）は未実装のままとする。
...
- POSIX cleanupはfdトラバーサルからpath unlinkへフォールバックせず、途中のdirectory消失はartifact absentとして扱う。file missingでもparts directoryのfsync完了を確認し、fsync失敗は`CLEANUP_FAILED`と次回retryを残す。通常・cancel・worker例外の終端経路はmanifest生成またはrepair pendingへ統合し、counterを取得できない場合はclaim条件付きの`STALE`／`manifest_repair_state=PENDING`へ退避する。
- manifest warningを最初にDBへ記録するtransactionで、修復後のterminal statusと固定error codeを専用列へ保存する。`0036_phase8c_manifest_repair_intent`は既存の0035 repair recordをbackfillし、`0037_phase8c_manifest_repair_backfill`はitem状態・cancel intent・既存error codeから曖昧なlegacy targetを再分類する。`0038_phase8c_legacy_manifest_recovery`は、旧アプリがFAILED manifestをDB確定済みのまま残したケースをDB状態だけで選別し、全成功系はcompleted、成功＋失敗系はpartially_completed、全失敗系は変更せず、修復可能な行だけを`STALE`／`PENDING`へ戻す。`0039_phase8c_manifest_orphan_scan`はterminal jobごとの走査時刻を永続化し、先頭32件の再取得によるstarvationを防ぐ。manifest参照commit後もPENDING／REPAIRING markerをterminal commitまで保持し、stale復旧、queued／stale／repair中のcancel、cleanup warning／claim／retry scheduleが残る間のmanifest・terminal化抑止を回帰テストで確認する。terminal jobのmanifest orphanはDB参照を保持対象として、起動時とschedulerのbounded reconciliationでgeneration basename、regular file、symlink、project root、Linux fd identityを検証して回収する。Phase 8Cでも過去migrationを変更しない。
- 直近のWindowsローカル検証では`ruff format --check .`、`ruff check .`、`mypy src`、`pytest`が成功し、`pytest`は388 passed、44 skipped、96 warningsだった。Linux固有のfd／symlink race系テストはWindows環境のためskipとして扱う。GitHub Actions `ci #67`（Ubuntu Linux、commit `ad078e7`）では`mypy`が成功し、`pytest`は432 passed、92 warningsで、対象7テストと0039公平走査テストを含めて成功した。Phase 8B／0038以前のmigrationは変更しない。
