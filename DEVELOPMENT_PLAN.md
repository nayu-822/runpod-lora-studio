...
- manifest warningを最初にDBへ記録するtransactionで、修復後のterminal statusと固定error codeを専用列へ保存する。`0036_phase8c_manifest_repair_intent`は既存の0035 repair recordをbackfillし、`0037_phase8c_manifest_repair_backfill`はitem状態・cancel intent・既存error codeから曖昧なlegacy targetを再分類する。manifest参照commit後もPENDING／REPAIRING markerをterminal commitまで保持し、stale復旧、queued／stale／repair中のcancel、cleanup warning／claim／retry scheduleが残る間のmanifest・terminal化抑止を回帰テストで確認する。Phase 8Cでも過去migrationを変更しない。
- 最新HEADでは`ruff format --check .`、`ruff check .`、`mypy src`が成功し、全体`pytest -q`は379 passed、44 skipped、85 warningsだった。retry／cancelの`.part`保持、claim commit前停止、unlink後の再開、cleanup backlog 65件のdrain、warning backoff、通常import cleanup失敗、manifest reference解除失敗時のfile保持、manifest directory replacement、migration downgrade／re-upgradeを確認した。WindowsではLinux fd／symlink race系テストがskipされる。Phase 8Bでは過去migrationを変更せず、`0034_phase8b_cleanup_retry_schedule`を`0033_phase8b_part_cleanup_claims`の子として追加した。
