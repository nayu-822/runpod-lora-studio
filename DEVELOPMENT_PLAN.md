...
- 直近のWindowsローカル検証では`ruff format --check .`、`ruff check .`、`mypy src`、`pytest`を実行し、Linux固有のfd／symlink race系テストはWindows環境のためskipとして扱う。Linux側の対象7テストとGitHub Actionsの最終結果は、0039とfixture修正を含むpush後に確認する。Phase 8B／0038以前のmigrationは変更しない。
