...
- completion markerはサイズ上限、同一remote bytesの再読SHA-256、DB記録hash、全provenance、完全な`export_files`集合、artifact transfer job、remote artifact実体を検証してからDBをcompletedへ更新する。既存markerは最新completed jobではなく、marker記載の完全一致transfer jobだけを受け入れる。
