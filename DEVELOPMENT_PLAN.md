...
- 終端化時の`.part`削除はitem／attemptの終端化と同じトランザクションで`PART_CLEANUP_PENDING`を記録し、commit後に専用claim token／時刻を取得して実行する。cleanup失敗、worker停止、claim喪失で保留状態を失わず、起動時またはstale import復旧後のboundedなcleanup recoveryだけが対象を再試行する。`FAILED retryable=true`と`CANCELED`の`.part`は`get_job()`や再開準備で削除せず、resume時はcleanup claimを原子的に解除または再開を拒否する

Phase 8Bの実装とレビュー指摘対応は、追加テストと品質チェックの成功を確認済み。manifestのfdトラバーサルではproject、acquisition、jobs、job、manifests各ディレクトリのdevice／inodeを保持し、DB更新前後に再トラバースして通常ディレクトリ置換とsymlink置換を検知する。`.part` cleanupは再開可能な保留状態とbounded claim recoveryを持つ。既存の`0032_phase8b_part_cleanup_warnings`を変更せず、その子migration `0033_phase8b_part_cleanup_claims`でcleanup claim列とindexを追加した。Google Driveへの成果物同期、完了manifestのDrive保存、PodのStop／TerminateはPhase 9で実装する。
...
- 最新HEADでは`ruff format --check .`、`ruff check .`、`mypy src`に加えて、retry／cancelの`.part`保持、cleanup race、bounded recovery、通常import cleanup失敗、manifest directory replacement、migration downgrade／re-upgradeを確認する。Phase 8Bでは過去migrationを変更せず、`0033_phase8b_part_cleanup_claims`を現在のHEADの子として追加する。
