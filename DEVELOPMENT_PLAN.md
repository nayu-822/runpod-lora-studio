...
- 最新HEADでは`ruff format --check .`、`ruff check .`、`mypy src`が成功し、`pytest -q`は364 passed、42 skipped、83 warningsだった。retry／cancelの`.part`保持、claim commit前停止、unlink後の再開、cleanup backlog 65件のdrain、warning backoff、通常import cleanup失敗、manifest reference解除失敗時のfile保持、manifest directory replacement、migration downgrade／re-upgradeを確認した。WindowsではLinux fd／symlink race系テストがskipされる。Phase 8Bでは過去migrationを変更せず、`0034_phase8b_cleanup_retry_schedule`を`0033_phase8b_part_cleanup_claims`の子として追加した。
